process.on('uncaughtException', (err) => {
  console.error('CRASHED! Uncaught Exception:', err);
  process.exit(1);
});

process.on('unhandledRejection', (reason, promise) => {
  console.error('CRASHED! Unhandled Rejection at:', promise, 'reason:', reason);
  process.exit(1);
});
...
// (We now initialize this dynamically inside the route using the user's BYOK)
...
app.post('/api/generate-random', async (req, res) => {
    console.log("Random recipe generator pressed");
    ...
    const reqString = (requirements && requirements.length > 0) ? requirements.join(", ") : "None";
    console.log(`Specific Requirements: ${reqString}`);
    
    // Construct the prompt
    const prompt = `You are a master chef AI. Generate ${count} customized recipes strictly adhering to these dietary requirements: ${reqString}. 
    ...
    Double check that ingredients and instructions are arrays of strings. No prose, no conversation, just the JSON.`;

    console.log("\n--- [ FULL PROMPT SENT TO AI AGENT ] ---");
    console.log(prompt);
    console.log("----------------------------------------");
    ...
    try {
        ...
        console.log("\nWaiting for Gemini AI Agent response...");
        const response = await ai.models.generateContent({
            model: 'gemini-2.5-flash',
            contents: prompt,
        });
        
        // Clean up possible markdown formatting in LLM output
        ...
        console.error("\n[!] AI Generation Error:", error.message || error);
        console.error("Make sure the user provided a valid GEMINI_API_KEY.\n");
        res.status(400).json({ error: "API key invalid or backend failed." });
    }
});
